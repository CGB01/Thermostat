...
void Adafruit_SH1106::begin(uint8_t vccstate, uint8_t i2caddr, bool reset) {
  ...
  if (sid != -1){
    pinMode(dc, OUTPUT);
    pinMode(cs, OUTPUT);
    csport      = (unsigned char*) portOutputRegister(digitalPinToPort(cs)); 	// [CGB]
    cspinmask   = digitalPinToBitMask(cs);
    dcport      = (unsigned char*) portOutputRegister(digitalPinToPort(dc));	// [CGB]
    dcpinmask   = digitalPinToBitMask(dc);
    if (!hwSPI){
      // set pins for software-SPI
      pinMode(sid, OUTPUT);
      pinMode(sclk, OUTPUT);
      clkport     = (unsigned char*) portOutputRegister(digitalPinToPort(sclk));	// [CGB]
      clkpinmask  = digitalPinToBitMask(sclk);
      mosiport    = (unsigned char*) portOutputRegister(digitalPinToPort(sid));		// [CGB]
